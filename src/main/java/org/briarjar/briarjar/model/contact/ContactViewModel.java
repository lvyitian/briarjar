package org.briarjar.briarjar.model.contact;

import org.briarjar.briarjar.model.GeneralException;
...
public class ContactViewModel {
	...
	private void checkAliasLength(String alias) throws GeneralException {
		if (alias.isBlank() || alias.length() > MAX_AUTHOR_NAME_LENGTH)
			throw new GeneralException("Invalid alias length (must be shorter" +
					" then "+MAX_AUTHOR_NAME_LENGTH+" characters and not be " +
					"blank)");
	}

	public Collection<Contact> getAcceptedContacts() throws DbException {
		return contactManager.getContacts();
	}

	public String getLink() throws DbException {
		return contactManager.getHandshakeLink();
	}

	public Collection<Pair<PendingContact, PendingContactState>>
			getPendingContacts() throws DbException {
		return contactManager.getPendingContacts();
	}

	public void removeAcceptedContact(ContactId c) throws DbException {
